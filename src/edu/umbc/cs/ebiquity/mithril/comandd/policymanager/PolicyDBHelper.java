...
import edu.umbc.cs.ebiquity.mithril.comandd.policymanager.util.AppInfo;
import edu.umbc.cs.ebiquity.mithril.comandd.policymanager.util.DefaultDataLoader;
...
import edu.umbc.cs.ebiquity.mithril.comandd.policymanager.util.UserContext;
/**
...
//	private final static String POLICY_TABLE_INDEX = "polindex";
	...
	public int addApplication(SQLiteDatabase db, AppInfo anAppInfo) {
		...
		try{
            ...
            Log.e(EbAndMWApplication.getDebugTag(), "Error inserting " + values, e);
            return -1;
		}
		return 1;
	}
			...
			if (cursor.moveToFirst()) {
				do {
					ProvInfo provInfo = new ProvInfo(
							...
							cursor.getString(4),
							cursor.getString(5));
					Log.v(EbAndMWApplication.getDebugTag(), provInfo.toString());
