package edu.umbc.cs.ebiquity.mithril.comandd.policymanager.util;
...
public class DefaultDataLoader {
	...
	private void policyLoad() {
		...
		if(indexOfKnownApp >= 0) {
			int count = 0;
			setValues(count++, EbAndMWApplication.getConstImages(), applications.get(indexOfKnownApp));
			setValues(count++, EbAndMWApplication.getConstFiles(), applications.get(indexOfKnownApp));
			setValues(count++, EbAndMWApplication.getConstVideos(), applications.get(indexOfKnownApp));
			setValues(count++, EbAndMWApplication.getConstAudios(), applications.get(indexOfKnownApp));
			setValues(count++, EbAndMWApplication.getConstContacts(), applications.get(indexOfKnownApp));
		}
	}
	private int findTheIndex() {
		int i = 0;
		for(AppInfo app : applications) {
			if(app.isMatch(EbAndMWApplication.getConstAppForWhichWeAreSettingPolicies()))
				return i;
			i++;
		}
		return -1;
