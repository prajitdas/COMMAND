package edu.umbc.cs.ebiquity.mithril.command.contentprovider.util.fakecontentproviders;
...
public class Contacts extends ContentProvider {
	 ...
	 */
	static final String PROVIDER_NAME = COMMANDApplication.getConstFakeAuthorityPrefix()
	...
	static{
		...
		uriMatcher.addURI(PROVIDER_NAME, COMMANDApplication.getConstContacts(), CONTACTS);
		uriMatcher.addURI(PROVIDER_NAME, COMMANDApplication.getConstContacts()+"/#", CONTACTS_ID);
		uriMatcher.addURI(PROVIDER_NAME, COMMANDApplication.getConstContacts()+"/#/data", CONTACTS_ID_DATA);
		uriMatcher.addURI(PROVIDER_NAME, COMMANDApplication.getConstContacts()
				...
				+COMMANDApplication.getConstSlash()+"#", CONTACTS_LOOKUP_ID);
		uriMatcher.addURI(PROVIDER_NAME, "data", CONTACTS_DATA);
		uriMatcher.addURI(PROVIDER_NAME, "data"+COMMANDApplication.getConstSlash()+"#", CONTACTS_DATA_ID);
	...
	*/
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			Log.v(COMMANDApplication.getDebugTag(), "came into onCreate for Contacts!");
			loadDefaultData(db);
		}
...
//				Log.v(EbAndMWApplication.getDebugTag(), "came into loadDefaultData for Contacts!");
			} catch (SQLException e) {
	            Log.e("error", "Error inserting " + values, e);
				Log.v(COMMANDApplication.getDebugTag(), "came into exception for Contacts!");
	            return -1;
			}
			values.clear();
